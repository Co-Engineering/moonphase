...
def render(methods: AuthMethods) -> str:
    ...
    # Neither provider will redirect to a bare IP, and usable()/incomplete()
    # already say so on the settings screen — but that is only a hint to the
    # UI. Gating the actual GOTRUE_EXTERNAL_*_ENABLED value on it too, not
    # just the stored checkbox, means clearing the domain and saving *that*
    # screen actually turns a configured provider off at GoTrue, instead of
    # leaving it live with working credentials while Moonphase's own sign-in
    # page simply stops mentioning it.
    domain = has_domain(methods.public_url)
    google_enabled = methods.google_enabled and domain
    microsoft_enabled = methods.microsoft_enabled and domain
    # GoTrue appends /oauth2/v2.0/authorize (and /token) to this itself — see
    # provider.chooseHost/provider/azure.go in the gotrue/auth source. A
    # trailing /v2.0 here used to double up with that and send every
    # Microsoft sign-in to .../<tenant>/v2.0/oauth2/v2.0/authorize, which
    # Microsoft has never served: the feature could not work in any
    # configuration until this was caught.
    azure_url = f"https://login.microsoftonline.com/{tenant}"
    lines = [
        ...
        "",
        f"GOTRUE_EXTERNAL_GOOGLE_ENABLED={_quote(str(google_enabled).lower())}",
        ...
        "",
        f"GOTRUE_EXTERNAL_AZURE_ENABLED={_quote(str(microsoft_enabled).lower())}",
