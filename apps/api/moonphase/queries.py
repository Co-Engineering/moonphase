...
async def personal_org_id_for_user_privileged(
    ...
    caller of this: it knows a session's owner from the row it is reconciling,
    not from a claim on its own connection, so it has to name the user
    explicitly rather than lean on `auth.uid()`. Requires a privileged
